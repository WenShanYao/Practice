...
```{r ans1b3}
c(hwGrades$學號) -> hwGrades$學號
c (classSurvey$學號) -> classSurvey$學號
...
table(classSurvey$性別)/sum(table(classSurvey$性別)) -> tbSex
tbSex
```
...
```{r ans12}
str_detect(classSurvey$課外活動,"團隊活動") -> hasGroupActivity
...
table(str_detect(classSurvey$課外活動,"團隊活動"))/sum(isFemale)->tbFemale
tbFemale
```
...
mean(hwGradesSubsample$平均成績[(hwGradesSubsample$女性==T & hwGradesSubsample$有團活==T)]) -> meanGradeFemaleWithAct
...
```{r ans23}
mean(hwSubmit3$截止時差[hwSubmit3$女性==T]) -> FemaleTime
mean(hwSubmit3$截止時差[hwSubmit3$女性==F]) -> MaleTime
